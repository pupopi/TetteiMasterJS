...
window.onload = function() {
    readFile("./patterns.json", function(jsonObj, error) {
        if(error) {         // ファイルが読み込めない場合はパターンメニューを作成しない
...
view.create = function(nx, ny, width, height) {
    ...
    view.layer[1].addEventListener("click", function(e) {
        var ix = Math.floor(e.offsetX/view.cellWidth);      // セルのｘ方向の番号
        var iy = Math.floor(e.offsetY/view.cellHeight);     // セルのｙ方向の番号
...
// 格子戦を描画するメソッド
view.drawLatticle = function() {
    // 各レイヤーのCanvasをリセット
    for(var i=0; i<view.layer.length; i++) {
        view.layer[i].width = view.layer[i].width;
    }
    // レイヤー1に格子を描く。格子はnxが150未満のときに描く
    if(view.nx<150) {
        var c = view.ctx[1];
...
controls.step = function(state) {
    var input = elt("inpue", { type: "button", value: "次へ" });
    input.addEventListener("click", function(e) {
