...
// * circleFull: 塗りつぶした円を描くツール
paintTools.circleFull = function(e, ctx) {
    var img = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
    var p = relativePosition(e, ctx.canvas);
    setDragListeners(ctx, img, function(q) {
        var dx = q.x - p.x;
...
var colorInput;     // Firefox用のchangeイベント対策用にinpur[type="color"]オブジェクトを格納
// * ペイントツールの選択
...
// * 色の選択肢(線と塗りの両方決定 → 必要に応じて別々のコントロールにしてください)
controls.color = function(ctx) {
    var input = colorInput = elt("input", {type: "color"});
    var label = elt("label", null, "色:", input);
    input.addEventListener("change", function(e) {     // 注: Firefoxでがchangeイベントが発生しない
...
controls.brushsize = function(ctx) {
    var size = [1,2,3,4,5,6,8,10,12,14,16,18,20,24,28];
    var select = elt("select", null);
...
controls.filter = function(ctx) {
    ...
    for( var name in filterTools ) {
        select.appendChild(elt("optioin", {value: name}, name));
    }
...
function loadImageURL(ctx, url) {
    ...
    image.onload = function() {
        ...
        var wshift = (ctx.canvas.with - factor*this*width   )/2;
